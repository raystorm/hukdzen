...
describe('FileControl', () => {
  it('renders as expected when file is uploading', () => {
    ...
    expect(
      container.getElementsByClassName(`${ComponentClassName.FileUploaderFileWrapper}`)
    ).toHaveLength(1);

    expect(
      container.getElementsByClassName(`${ComponentClassName.FileUploaderFileStatus}`)
    ).toHaveLength(1);

    expect(
      container.getElementsByClassName(`${ComponentClassName.FileUploaderLoader}`)
    ).toHaveLength(1);

    expect(
      container.getElementsByClassName(`${ComponentClassName.FileUploaderFileImage}`)
    ).toHaveLength(0);
  });
