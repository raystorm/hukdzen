...
import { FileListHeader, FileListHeaderProps } from '../index';
...
describe('FileListHeader', () => {
  it('renders as expected when uploads are in progress', async () => {
    ...
    expect(
      container.getElementsByClassName(`${ComponentClassName.FileUploaderPreviewerText}`)
    ).toHaveLength(1);

    expect(
      await findByText(
        headerProps.displayText.getRemainingFilesText(headerProps.remainingFilesCount)
      )
    ).toBeVisible();
