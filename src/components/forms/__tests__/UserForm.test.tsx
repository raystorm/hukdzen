...
describe("UserForm", () => {
  ...
  test("UserForm renders correctly for Admin User", async () => {
    ...
    expect(uBoxes).toBeInTheDocument();
    expect(
      screen.getByText(`${printBoxRole(TEST_USER.boxRoles[0])}`)
    ).toBeInTheDocument();
  });
  ...
  test("UserForm email validation works", async () => {
    ...
    await waitFor(() => {
      expect(getEmailField()).toHaveValue(validEmail);
    });
  ...
  test("UserForm able to set Name", async () => {
    ...
    await waitFor(() => {
      expect(screen.getByLabelText(startsWith("Name"))).toHaveValue(
        changedValue
      );
    });
  });
  ...
  test("UserForm able to set Waa", async () => {
    ...
    await waitFor(() => {
      expect(screen.getByLabelText(startsWith("Waa"))).toHaveValue(
        changedValue
      );
    });
  });
