...
import { request, response } from '../updateUserGuarded.js';

describe('updateUserGuarded', () =>
{
   ...
   describe('request', () =>
   {
      ...
      {
         const ctx = { arguments: { input: { name: 'John Doe', email: 'john@example.com' } } };
         expect(() => request(ctx)).toThrow('ValidationError: id is required');
      });

      it('validates name is required', () =>
      {
         const ctx = { arguments: { input: { id: 'user-1', email: 'john@example.com' } } };
         expect(() => request(ctx)).toThrow('ValidationError: name is required');
      });

      it('validates email is required', () =>
      {
         const ctx = { arguments: { input: { id: 'user-1', name: 'John Doe' } } };
         expect(() => request(ctx)).toThrow('ValidationError: email is required');
      });
      ...
      it('creates UpdateItem operation with all fields', () =>
      {
         const ctx = {
            arguments: {
               input: {
                  ...
                  waa: 'Waa Name',
                  isAdmin: true,
                  emailPreferences: { allOptOut: true }
               }
            }
         };
         ...
         expect(result.key).toEqual({ id: 'user-1' });
         expect(result.update.expression).toBe(
            'SET #name = :name, #clan = :clan, #waa = :waa, #email = :email, #isAdmin = :isAdmin, #emailPreferences = :emailPreferences, #updatedAt = :updatedAt'
         );
      ...
      it('includes condition to ensure item exists', () =>
      {
         const ctx = {
            arguments: {
               input: { id: 'user-1', name: 'John Doe', email: 'john@example.com' }
            }
         };
   ...
   describe('response', () =>
   {
      ...
      {
         const ctx = { result: { id: 'user-1', name: 'John Doe' } };
         expect(response(ctx)).toEqual({ id: 'user-1', name: 'John Doe' });
      });
